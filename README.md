...
- LLM 建議：按「生成改善建議」生成改善建議。

#### Streamlit 操作步驟
1) 在側邊欄選擇或上傳模型檔（`.keras`/`.h5`）。
2) 調整「判定門檻」，確認「氣候解讀」開啟（預設啟用）。
3) 在主畫面上傳影像。
4) 系統自動推論，查看機率、判定與 Grad-CAM 熱力圖。
5) 按「生成改善建議」取得氣候風險改善建議。

#### 使用功能
- 模型選擇/上傳：支援切換不同模型權重。
- 判定門檻：控制「有仙人掌」判定的敏感度。
- Grad-CAM 熱力圖：顯示模型關注區域。
- 氣候風險提示：依偵測結果提供基本解讀。
- 本地 LLM 改善建議：按鈕觸發生成建議，不需 API key。
